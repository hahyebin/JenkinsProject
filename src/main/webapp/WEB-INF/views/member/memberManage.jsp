...
<script>
	...
	function fnAddMember(){
		$('#insert_btn').click(function(){
			...
			$.ajax({
				url : '/jenkinsproject/api/members',   // 목록과 똑같은 주소가... 대신 메소드가 달라진다.
				type : 'post',
	...
	function fnFindMember(){
		...
		$.ajax({
			url: '/jenkinsproject/api/members/'+$(this).prev().val(),    // url(경로)에 각 번호를 적었기 때문에 보내는 데이터는 없다.
			type: 'get',
			dataType: 'json',
			success: function(map){
				if (map.member != null ){
	...
	function fnModifyMember(){
		$('#update_btn').click(function(){
			...
			$.ajax({
				url: '/jenkinsproject/api/members',   // 목록과 똑같은 주소가... 대신 메소드가 달라진다.(PUT)
				type: 'put',
				contentType: "application/json",
			    data : member,
			    dataType: 'json',
